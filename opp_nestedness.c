...
int fj(int I, int N, float p){
    ...
    int n = N-1;
    float q = 1-p;
    return (int)(N - pow(i, p) * pow(n, q));
}

void nestedness_exp(int N, float p, float eps){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w;
...
float nestedness_energy(int N, float p){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w, e, e0;
...
void print_compression_rate(int N, float p){
    ...
    for(i = 1; i <= N; i++){
        for(j = 1; j <= N; j++){
            if(s[i][j] != s[j][i]){
                s[i][j] = s[j][i];   /* should not be needed */ 
            }
        }
    }
    Mout = deg[0] - M1;
...
int main(int argc, char *argv[]){
    ...
    sprintf(xfile, "nestedness_x%.2f_%s", p, network);
    sprintf(bfile, "nestedness_b%.2f_%s", p, network);
    sprintf(sfile, "nestedness_s%.2f_%s", p, network);
    sprintf(rfile, "nestedness_r%.2f_%s", p, network);
