...
int f3j(int I, int N, float p){
    ...
    return (int)((2*N)/5 + pow(r,p)*n);
}
int f4j(int I, int N, float p){
    float i = I-(3*N)/5;
    float n = N/5-1;
    float r = i/n;

    return (int)((3*N)/5 + pow(r,p)*n);
}
int f5j(int I, int N, float p){
    float i = I-(4*N)/5;
    float n = N/5-1;
    float r = i/n;

    return (int)((4*N)/5 + pow(r,p)*n);
}
void pentapole_exp(int N, float p, float eps){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w;
    ...
    for(i = 1; i <= N; i++){
        ...
        for(j = (3*N)/5; j < (4*N)/5; j++){
            w = -(eps * x[i][j]);
            for(l = f4j(j, N, 1./p); l <= f4j(j, N, p); l++){
                 for(k = 1; k <= deg[i]; k++){
                    nk = blist[i][k];
                    w += x[nk][l];
                }
        ...
        for(j = (4*N)/5; j <= N; j++){
            w = -(eps * x[i][j]);
            for(l = f5j(j, N, 1./p); l <= f5j(j, N, p); l++){
                 for(k = 1; k <= deg[i]; k++){
                    nk = blist[i][k];
                    w += x[nk][l];
                }
...
float pentapole_energy(int N, float p){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w, e, e0;
...
void print_compression_rate(int N, float p){
    ...
    for(i = (3*N)/5; i < (4*N)/5; i++){
        for(j = f4j(i, N, 1./p); j <= f4j(i, N, p); j++){
            if(s[i][j] == 1){
                M1 += 1;
                s[i][j] = 2;
            }
    ...
    for(i = (4*N)/5; i <= N; i++){
        for(j = f5j(i, N, 1./p); j <= f5j(i, N, p); j++){
            if(s[i][j] == 1){
                M1 += 1;
                s[i][j] = 2;
            }
...
int main(int argc, char *argv[]){
    ...
    sprintf(xfile, "pentapole_x%.2f_%s", p, network);
    sprintf(bfile, "pentapole_b%.2f_%s", p, network);
    sprintf(sfile, "pentapole_s%.2f_%s", p, network);
    sprintf(rfile, "pentapole_r%.2f_%s", p, network);
