...
int f4j(int I, int N, float p){
    ...
    return (int)((3*N)/4 + pow(r,p)*n);
}
void quadrupole_exp(int N, float p, float eps){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w;
...
float quadrupole_energy(int N, float p){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w, e, e0;
...
int main(int argc, char *argv[]){
    ...
    sprintf(xfile, "quadrupole_x%.2f_%s", p, network);
    sprintf(bfile, "quadrupole_b%.2f_%s", p, network);
    sprintf(sfile, "quadrupole_s%.2f_%s", p, network);
    sprintf(rfile, "quadrupole_r%.2f_%s", p, network);
