...
void triangular_exp(int N, int Q, float eps){
    int i, j, k, l, nk, q;
    int lmin, lmax;
    float w, r;
    r = ((float)N)/((float)Q);
...
float triangular_energy(int N, int Q){
    int i, j, k, l, nk, q;
    int lmin, lmax;
    float w, e = 0, e0 = 0, r;
    r = ((float)N)/((float)Q);
...
int main(int argc, char *argv[]){
    ...
    sprintf(xfile, "triangular_x%d_%s", Q, network);
    sprintf(bfile, "triangular_b%d_%s", Q, network);
    sprintf(sfile, "triangular_s%d_%s", Q, network);
    sprintf(rfile, "triangular_r%d_%s", Q, network);
