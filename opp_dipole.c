...
int f2j(int I, int N, float p){
    ...
    return (int)(N/2 + pow(r,p)*n);
}
void dipole_exp(int N, float p, float eps){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w;
...
float dipole_energy(int N, float p){
    int i, j, k, l, nk;
    int lmin, lmax;
    float w, e, e0;
...
int main(int argc, char *argv[]){
    ...
    sprintf(xfile, "dipole_x%.2f_%s", p, network);
    sprintf(bfile, "dipole_b%.2f_%s", p, network);
    sprintf(sfile, "dipole_s%.2f_%s", p, network);
    sprintf(rfile, "dipole_r%.2f_%s", p, network);
